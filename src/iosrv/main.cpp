...
int main(int argc, char * argv[])
{
	if(argc != 2)
	{
		std::cerr << "arg invalid: iosrv config_path" << std::endl;
		return 1;
	}
	...
	UnitManager::Instance()->Init(config["IOSrv"]["interval"].value_or(1));
	...
	UnitManager::Instance()->Register("IOSRV", std::move(std::make_shared<IOSrv>(config["IOSrv"]["id"].value_or(INVALID_NODE_ID), config)));
